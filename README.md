...
Averaged over 8 seeds, 250 simulated steps, at three levels of informed order
flow:

| Informed flow | Gross edge / contract | Markout / contract | **Edge retained** |
|---|---:|---:|---:|
| none | 3.1451 | 3.0917 | **98.8%** |
| 30% | 3.0436 | 2.4062 | **79.0%** |
| 75% | 3.4047 | 2.2148 | **63.4%** |

The maker quotes the same spread in every regime and earns essentially the same
gross edge — but keeps 98.8%, 79.0%, then 63.4% of it. That gap is adverse
selection, and it is invisible in revenue. The P&L decomposition that produces
those numbers reports its own **unexplained residual at 2.3–2.5% of gross**,
so it is describing the P&L rather than tiling it.

Along the way the surface fit recovers a skew that flattens monotonically with
maturity (`rho` from −0.75 to −0.45) from a chain generated by a model whose
functional form it was never told, to within 1.3e-3 absolute volatility.

- **[DESIGN.md](DESIGN.md)** — every decision, and the ten defects the process
  caught
- **[BENCHMARKS.md](BENCHMARKS.md)** — measured numbers, including why the
  wing inversion costs 9.8x the at-the-money one

## Architecture

```mermaid
flowchart TD
    A[Option chain<br/>synthetic, seeded] -->|regress call-put on strike| B[Forward &amp; discount<br/>parity.hpp]
    B --> C[Implied volatility<br/>safeguarded Newton<br/>implied_volatility.hpp]
    A --> C
    C -->|out-of-the-money only| D[SVI slice fit<br/>svi.hpp]
    D --> E{Arbitrage checks}
    E -->|Durrleman density| F[Vol surface<br/>surface.hpp]
    E -->|calendar in total variance| F
    F --> G[Portfolio risk<br/>bucketed vega<br/>portfolio.hpp]
    G --> H[Quoting engine<br/>skew &amp; width<br/>quoting.hpp]
    F --> H
    H --> I[Simulation<br/>simulation.hpp]
    G --> I
    I --> J[P&amp;L attribution<br/>+ adverse selection]
```

## Status

Built incrementally, one milestone at a time. **110 tests**, green across gcc
and clang in Debug with ASan/UBSan and in Release, all under `-Werror`.
...
| M8 | Benchmarks, `DESIGN.md`, `BENCHMARKS.md` | **Done** |
...
Six Google Benchmark executables under `bench/`, plus
[DESIGN.md](DESIGN.md) and [BENCHMARKS.md](BENCHMARKS.md).

Every figure in BENCHMARKS.md was captured by running the benchmarks, not
estimated. CI compile-checks them on the Release legs with `-Werror` and never
runs them: a shared runner produces timings that cannot be trusted, and an
untrustworthy number is worse than no number.

The three results worth knowing:

| Result | Number |
|---|---:|
| Deep-wing implied vol vs at-the-money | **9.8x** (1637 ns vs 167 ns) |
| SVI fit at half search resolution | **5.1x** faster (61.3 µs vs 313 µs) |
| Portfolio risk, 10 → 4,000 positions | **linear**, 3.5M positions/s flat |

The first justifies the solver's design directly. Near the money Newton
converges quadratically; in the wings vega collapses to ~1.5e-15 and the
safeguard forces bisection, which needs about forty iterations instead of a
handful. The cost is real and worth paying — the alternative is not a faster
answer but no answer at all in the wings, which is where a surface most needs
points. The ill-conditioned deep in-the-money case costs 3.93 ns, faster than a
single valuation, because refusing to answer is the cheapest path through the
solver.

BENCHMARKS.md also documents a benchmark that was measuring itself:
`BM_ComputeRisk` reported a suspiciously flat 17.6 µs from 100 to 5,000
positions because its contract generator repeated after 60 contracts and the
book netted the rest away.
...
./build-bench/bench/bench_implied_vol
```
